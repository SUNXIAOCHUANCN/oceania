...
private const string SAVE_KEY_FIRST_START = "GlobalTimeSystem_FirstStart";
    ...
    public int TotalPhases { get; private set; } // 从游戏开始经过的phaseDuration数量
    ...
    void Update()
    {
        ...
        if (_lastPhase != CurrentPhase)
        {
            ...
            // 广播当前的totalPhases
            OnPhaseChangedWithTotalPhases?.Invoke(CurrentPhase, TotalPhases);
    ...
    void SyncStateFromAbsoluteTime()
    {
        ...
        // 计算总共经历的phaseDuration数量
        TotalPhases = CycleCount * 4 + phaseIndex;
    }
    ...
    public void ResetTime()
    {
        ...
        OnTimeReset?.Invoke();
        OnPhaseChanged?.Invoke(CurrentPhase);
        OnPhaseChangedWithTotalPhases?.Invoke(CurrentPhase, TotalPhases); // 广播totalPhases为0
    ...
    {
        // 重置到第一个phaseDuration（即Crescent阶段）
        TotalElapsedTime = 0f;
        SyncStateFromAbsoluteTime();
        ...
        OnPhaseChangedWithTotalPhases?.Invoke(CurrentPhase, TotalPhases); // 广播totalPhases为0
        OnNewCycle?.Invoke(CycleCount);
        Debug.Log("[GlobalTimeSystem] Moon phase reset.");
    }
    ...
    public event Action<MoonPhase, int> OnPhaseChangedWithTotalPhases; // 添加带totalPhases参数的事件
